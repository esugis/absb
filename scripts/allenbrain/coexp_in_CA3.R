# This script computes co-expression in CA3 tissue from Allen Brain Atlas.
# Co-expression is computed over 6 datasets of gene expression in healthy brains.

# Set working directory
setwd("~/absb/results/allenbrain/tissues_rdata/CA3")
tissue_path <- "~/absb/results/allenbrain/tissues_rdata/CA3"
...
# Load CA3 dataset
load(file="CA3_preprocessed.RData")
...
sink_filenametissue<-sprintf("%s.txt", "foreach_sink_per_probeset_CA3")
...
# Add ensg IDs for CA3 co-expression dataset
CA3_filt <- filtered_coexpression

CA3_filt_ensg1 <- merge(CA3_filt, genes_aba_ensg, by.x = "probeset.A", by.y = "probe_id", all = F)
CA3_filt_ensg12 <- merge(CA3_filt_ensg1, genes_aba_ensg, by.x = "probeset.B", by.y = "probe_id", all = F)

# Select only ensg1 ensg2 score and add type of interaction and data source
library(plyr)
colnames(CA3_filt_ensg12)[c(6,8)] <- c("ensg1", "ensg2")
...
CA3 <- cbind(CA3, data_source = "ABA")

# Rename the columns
colnames(CA3) <- c("ensg1","ensg2", "score", "interaction_type","data_source")

## Remove the duplicated undirrescted eCA3es with the same score.
...
head(CA3)
CA3_coexp_int <- CA3
CA3_coexp_int$data_source <- "ABA_CA3"
save(CA3_coexp_int, file = "~/absb/results/allenbrain/CA3_coexp_int.RData")
write.table(CA3_coexp_int,file = "~/absb/results/allenbrain/CA3_coexp_int.txt", quote = F, sep = "\t",row.names = F)
